...
class BitcoinCoreWallet extends wallet_provider_1.WalletProvider {
    ...
    async walletCreateFundedPsbt(inputs, outputs, locktime = 0, options = {}, bip32derivs = true) {
        /*
        if (!options) {
            options = {}
        }
    ...
    async analyzePsbt(psbtHex) {
        return this.client.analyzePsbt(Buffer.from(psbtHex, "hex").toString("base64"));
    }
    async signPsbt(psbtHex, sign = true, sighashtype = 'DEFAULT', bip32derivs = true, finalize = true) {
        console.log('Signing PSBT with hex:', psbtHex);
        const signedPsbt = await this.client.walletProcessPsbt(Buffer.from(psbtHex, 'hex').toString('base64'), sign, sighashtype, bip32derivs, finalize);
    ...
    async signPsbts(psbtsHexes, sign = true, sighashtype = 'DEFAULT', bip32derivs = true, finalize = true) {
        const signedPsbts = [];
        for (const psbtHex of psbtsHexes) {
            const signedPsbt = await this.signPsbt(psbtHex, sign, sighashtype, bip32derivs, finalize);
